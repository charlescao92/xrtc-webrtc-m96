...
#include "modules/rtp_rtcp/include/rtp_header_extensions.h"
...
bool AbsoluteSendTime::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() != 3)
    return false;
  *time_24bits = webrtc::ByteReader<uint32_t, 3>::ReadBigEndian(data.data());
  return true;
}
...
bool AbsoluteSendTime::Write(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint32_t, 3>::WriteBigEndian(data.data(), time_24bits);
  return true;
}
...
bool AbsoluteCaptureTimeExtension::Write(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint64_t>::WriteBigEndian(data.data(),
                                       extension.absolute_capture_timestamp);

  if (data.size() != kValueSizeBytesWithoutEstimatedCaptureClockOffset) {
    webrtc::ByteWriter<int64_t>::WriteBigEndian(
...
bool CsrcAudioLevel::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() > webrtc::kRtpCsrcSize) { // 15
    return false;
  }
...
bool CsrcAudioLevel::Write(rtc::ArrayView<uint8_t> data,
                           rtc::ArrayView<const uint8_t> csrc_audio_levels) {
  RTC_CHECK_LE(csrc_audio_levels.size(), webrtc::kRtpCsrcSize); // 15
...
bool TransmissionOffset::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() != 3)
    return false;
  *rtp_time = webrtc::ByteReader<int32_t, 3>::ReadBigEndian(data.data());
  return true;
}
...
bool TransmissionOffset::Write(rtc::ArrayView<uint8_t> data, int32_t rtp_time) {
  ...
  webrtc::ByteWriter<int32_t, 3>::WriteBigEndian(data.data(), rtp_time);
  return true;
}
  ...
  *feedback_request = absl::nullopt;
  if (data.size() == kValueSizeBytes) {
    uint16_t feedback_request_raw =
        webrtc::ByteReader<uint16_t>::ReadBigEndian(data.data() + 2);
...
bool TransportSequenceNumberV2::Write(
  ...
  if (feedback_request) {
    ...
    webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + 2, feedback_request_raw);
  }
  return true;
}
...
bool VideoOrientation::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() != 1)
    return false;
  *rotation = webrtc::ConvertCVOByteToVideoRotation(data[0]);
  return true;
}
...
bool VideoOrientation::Write(rtc::ArrayView<uint8_t> data,
                             webrtc::VideoRotation rotation) {
  RTC_DCHECK_EQ(data.size(), 1);
  data[0] = webrtc::ConvertVideoRotationToCVOByte(rotation);
  return true;
}
...
bool PlayoutDelayLimits::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() != 3)
    return false;
  uint32_t raw = webrtc::ByteReader<uint32_t, 3>::ReadBigEndian(data.data());
...
bool PlayoutDelayLimits::Write(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint32_t, 3>::WriteBigEndian(data.data(),
...
bool VideoTimingExtension::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  switch (data.size()) {
    ...
    case kValueSizeBytes:
      timing->flags = webrtc::ByteReader<uint8_t>::ReadBigEndian(data.data());
      break;
    default:
      return false;
  }

  timing->encode_start_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
      data.data() + kEncodeStartDeltaOffset - off);
  timing->encode_finish_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
      data.data() + kEncodeFinishDeltaOffset - off);
  timing->packetization_finish_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
      data.data() + kPacketizationFinishDeltaOffset - off);
  timing->pacer_exit_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
      data.data() + kPacerExitDeltaOffset - off);
  timing->network_timestamp_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
      data.data() + kNetworkTimestampDeltaOffset - off);
  timing->network2_timestamp_delta_ms = webrtc::ByteReader<uint16_t>::ReadBigEndian(
...
bool VideoTimingExtension::Write(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint8_t>::WriteBigEndian(data.data() + kFlagsOffset, timing.flags);
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + kEncodeStartDeltaOffset,
                                       timing.encode_start_delta_ms);
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + kEncodeFinishDeltaOffset,
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + kPacerExitDeltaOffset,
...
bool VideoTimingExtension::Write(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + offset, time_delta_ms);
  return true;
}
...
size_t ColorSpaceExtension::ParseHdrMetadata(rtc::ArrayView<const uint8_t> data,
  ...
  hdr_metadata->max_content_light_level =
      webrtc::ByteReader<uint16_t>::ReadBigEndian(data.data() + offset);
  offset += 2;
...
size_t ColorSpaceExtension::ParseChromaticity(
  ...
  uint16_t chromaticity_x_scaled = webrtc::ByteReader<uint16_t>::ReadBigEndian(data);
  uint16_t chromaticity_y_scaled =
      webrtc::ByteReader<uint16_t>::ReadBigEndian(data + 2);
...
size_t ColorSpaceExtension::ParseLuminance(const uint8_t* data,
                                           float* f,
                                           int denominator) {
  uint16_t luminance_scaled = webrtc::ByteReader<uint16_t>::ReadBigEndian(data);
...
size_t ColorSpaceExtension::WriteHdrMetadata(rtc::ArrayView<uint8_t> data,
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data() + offset,
                                       hdr_metadata.max_content_light_level);
  offset += 2;
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(
...
size_t ColorSpaceExtension::WriteLuminance(uint8_t* data,
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data, std::round(upscaled_value));
...
bool VideoFrameTrackingIdExtension::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  *video_frame_tracking_id = webrtc::ByteReader<uint16_t>::ReadBigEndian(data.data());
  return true;
}
...
bool VideoFrameTrackingIdExtension::Write(rtc::ArrayView<uint8_t> data,
                                          uint16_t video_frame_tracking_id) {
  RTC_DCHECK_EQ(data.size(), kValueSizeBytes);
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(data.data(), video_frame_tracking_id);
  return true;
}
