...
#include "modules/rtp_rtcp/include/rtp_video_layers_allocation_extension.h"
...
bool RtpVideoLayersAllocationExtension::Write(
  ...
  *write_at |= slb.max_rtp_stream_id << 4;
  ...
  if (allocation.resolution_and_frame_rate_is_valid) {
    for (const auto& spatial_layer : allocation.active_spatial_layers) {
      webrtc::ByteWriter<uint16_t>::WriteBigEndian(write_at, spatial_layer.width - 1);
      write_at += 2;
      webrtc::ByteWriter<uint16_t>::WriteBigEndian(write_at, spatial_layer.height - 1);
      write_at += 2;
...
bool RtpVideoLayersAllocationExtension::Parse(
  ...
  allocation->resolution_and_frame_rate_is_valid = true;
  for (auto& layer : allocation->active_spatial_layers) {
    layer.width = 1 + webrtc::ByteReader<uint16_t, 2>::ReadBigEndian(read_at);
    read_at += 2;
    layer.height = 1 + webrtc::ByteReader<uint16_t, 2>::ReadBigEndian(read_at);
    read_at += 2;
...
size_t RtpVideoLayersAllocationExtension::ValueSize(const webrtc::VideoLayersAllocation& allocation) {
  if (allocation.active_spatial_layers.empty()) {
    return 1;
  }
